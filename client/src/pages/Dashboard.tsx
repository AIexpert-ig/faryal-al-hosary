...
function StatCard({ icon, label, value, color, trend }: { icon: React.ReactNode; label: string; value: number | string; color: string; trend?: string }) {
  return (
    ...
    >
      ...
      <div>
        <div style={{ fontFamily: 'var(--font-sans)', fontWeight: 800, fontSize: '2rem', color: 'var(--color-dark)', lineHeight: 1 }}>
          {value}
        </div>
        <div style={{ fontFamily: 'var(--font-body)', fontSize: '0.8rem', color: '#888', marginTop: '0.25rem', letterSpacing: '0.05em' }}>
          {label}
        </div>
      </div>
    </div>
      ...
      <div style={{ width: '100%', height: '120px', display: 'flex', alignItems: 'flex-end', background: '#f4f4f4', borderRadius: '2px', overflow: 'hidden' }}>
        <div
          style={{
            width: '100%',
            height: `${pct}%`,
            background: 'linear-gradient(to top, #0d1310, #2a4a3a)',
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-bg)' }}>
      ...
      <div style={{ padding: '2.5rem 3rem', maxWidth: '1400px', margin: '0 auto' }}>
        ...
        {/* Error banner */}
        {error && (
          <div style={{ background: '#fff3cd', border: '1px solid #ffc107', borderRadius: '4px', padding: '0.75rem 1rem', marginBottom: '1.5rem', fontFamily: 'var(--font-body)', fontSize: '0.875rem', color: '#856404' }}>
            {error}
          </div>
        )}
        ...
        {loading ? (
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1.5rem', marginBottom: '2.5rem' }}>
              ...
              <div style={{ background: 'white', padding: '2rem', borderRadius: '2px', boxShadow: '0 1px 10px rgba(0,0,0,0.06)' }}>
                <div style={{ marginBottom: '1.5rem' }}>
                  <h3 style={{ fontFamily: 'var(--font-sans)', fontWeight: 700, fontSize: '1rem', color: 'var(--color-dark)', marginBottom: '0.25rem' }}>
                    Monthly Revenue
                  </h3>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.8rem', color: '#888' }}>Last 6 months</p>
                </div>
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-end', height: '160px' }}>
                  {revenue.map((r) => (
                    <RevenueBar key={r.month} month={r.month} amount={r.amount} maxAmount={maxRevenue} />
                  ))}
                </div>
              </div>
              ...
              <div style={{ background: 'white', padding: '2rem', borderRadius: '2px', boxShadow: '0 1px 10px rgba(0,0,0,0.06)' }}>
                <div style={{ marginBottom: '1.5rem' }}>
                  ...
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.8rem', color: '#888' }}>Distribution</p>
                </div>
                {[
                  { label: 'Available', value: stats.availableUnits, color: '#22c55e' },
                  { label: 'Active', value: stats.activeRentals, color: '#c9a96e' },
                  { label: 'Reserved', value: 8, color: '#3b82f6' },
                  { label: 'Cleaning', value: 4, color: '#a855f7' },
                ].map((item) => {
                  ...
                  return (
                    <div key={item.label} style={{ marginBottom: '0.75rem' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                        <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.8rem', color: '#555' }}>{item.label}</span>
                        <span style={{ fontFamily: 'var(--font-sans)', fontWeight: 600, fontSize: '0.8rem', color: 'var(--color-dark)' }}>{item.value}</span>
                      </div>
            ...
            <div style={{ background: 'white', borderRadius: '2px', boxShadow: '0 1px 10px rgba(0,0,0,0.06)' }}>
              ...
              {schedule.length === 0 ? (
                <div style={{ padding: '3rem', textAlign: 'center', fontFamily: 'var(--font-body)', color: '#888' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ background: '#fafafa' }}>
                      {['Code', 'Customer', 'Gown SKU', 'Type', 'Time'].map((h) => (
                        <th
                          key={h}
                          style={{
                  ...
                  <tbody>
                    {schedule.map((item, i) => (
                      ...
                      >
                        <td style={{ padding: '1rem 2rem', fontFamily: 'var(--font-body)', fontSize: '0.875rem', color: 'var(--color-dark)', fontWeight: 600 }}>
                          {item.rental_code}
                        </td>
                        <td style={{ padding: '1rem 2rem', fontFamily: 'var(--font-body)', fontSize: '0.875rem', color: '#555' }}>
                          {item.customer_name}
                        </td>
                        <td style={{ padding: '1rem 2rem', fontFamily: 'var(--font-body)', fontSize: '0.875rem', color: '#555' }}>
                          {item.gown_sku}
                        ...
                        <td style={{ padding: '1rem 2rem' }}>
                          <span
                            style={{
                              ...
                              borderRadius: '2px',
                              background: item.type === 'pickup' ? 'rgba(201,169,110,0.15)' : 'rgba(34,197,94,0.12)',
                              color: item.type === 'pickup' ? '#a07840' : '#166534',
                            }}
                          >
                            {item.type}
                          </span>
                        </td>
                        <td style={{ padding: '1rem 2rem', fontFamily: 'var(--font-body)', fontSize: '0.875rem', color: '#888' }}>
                          {item.time ?? '—'}
                        </td>
                      </tr>
                    ))}
